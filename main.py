...
svm_classifier.fit(tfs_training, y_train)

# Predict the response for test dataset
tfs_testing = Tfidf_Vectorizer.transform(x_test).astype('float64').todense()
y_pred = svm_classifier.predict(tfs_testing)

print("Accuracy:", metrics.accuracy_score(y_test, y_pred))

truncatedSVD = TruncatedSVD(100)
Xpca = truncatedSVD.fit_transform(tfs_testing)
...
case = Tfidf_Vectorizer.transform([review]).astype('float64')
print(svm_classifier.predict(case))
